...
class LSTMAdaBoost:
    ...
    def fit(self, X, y, epochs=30, batch_size=32, validation_split=0.1, verbose=1, model_save_dir='./intermediate_models'):
        """
        训练LSTM-AdaBoost模型

        参数:
        model_save_dir: 训练过程中保存中间模型的目录
        """
        # 数据标准化
        ...
        for i in range(self.n_estimators):
            ...
            # 每次训练完一个弱学习器后保存它，以防程序中断
            model.save(f"{model_save_dir}/lstm_estimator_intermediate_{i}.h5")
            print(f"中间模型 {i+1} 已保存到 {model_save_dir}/lstm_estimator_intermediate_{i}.h5")
    ...
    def plot_predictions(self, X, y_true, title="LSTM-AdaBoost预测结果", save_path=None):
        """
        绘制预测结果并选择性保存。
        ...
        y_true: 真实目标值。
        title: 图表标题。
        save_path: 如果提供，图表将保存到此路径而不是显示。
                   示例: 'predictions.png' 或 'plots/predictions.pdf'
        """
        y_pred = self.predict(X)
...
def main():
    ...
    # 设置一个目录用于保存训练过程中的中间模型
    intermediate_model_dir = './intermediate_models'
    model.fit(X, y, epochs=100, batch_size=32, verbose=1, model_save_dir=intermediate_model_dir)

    # 评估模型
    metrics = model.evaluate(X, y)

    # 绘制预测结果并保存到文件
    plot_save_path = './predictions.png'  # 指定保存图表的路径和文件名
    model.plot_predictions(X, y, title="LSTM-AdaBoost 电力负荷预测结果", save_path=plot_save_path)

    # 保存最终模型
    final_model_path = r'./final_model'
    os.makedirs(final_model_path, exist_ok=True)

    # 保存每个弱学习器
    for i, estimator in enumerate(model.estimators):
        estimator.save(f"{final_model_path}/lstm_estimator_{i}.h5")

    # 保存模型配置
    with open(f"{final_model_path}/model_config.txt", 'w') as f:
